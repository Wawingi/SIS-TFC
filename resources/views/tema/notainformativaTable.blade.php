    <tr>
        <td colspan="4"><span style="text-align:center;font-weight:bold"><i class="fas fa-info-circle mr-1"></i>NOTA INFORMATIVA SOBRE A DEFESA</span>@can('apagar_global_tutor')<a href="#" id="{{$notaInformativa->id}}" class="eliminarNotaInformativa float-right btn btn-danger btn-sm btn-rounded"><i class='fa fa-trash-alt mr-2'></i>Eliminar</a>@endcan</td>
    </tr>
    <tr>
    ...
    <tr>                
        ...
        <td><a href="#" class="dataprovapublica_edit" data-name="created_at" data-type="combodate" data-value="{{$notaInformativa->created_at}}" data-format="YYYY-MM-DD" data-viewformat="DD/MM/YYYY" data-template="DD/MM/YYYY" data-pk="{{$notaInformativa->id}}"></a></td>                
        <td style="background:#edeff1">Local da defesa</td>                
        <td><a href="#" class="local_edit" data-name="local" data-type="text" data-placeholder="Preenchimento obrigatório" data-pk="{{$notaInformativa->id}}" data-title="Informe o local">{{$notaInformativa->local}}</a></td>                
    </tr>                
                
    <tr>                
        <td style="background:#edeff1">Presidente</td>                
        <td><B>{{$notaInformativa->presidente}}</B></td>                
        <td style="background:#edeff1">Secretário</td>                
        <td><B>{{$notaInformativa->secretario}}</B></td>                
    </tr>                
             
    <tr>                
        <td style="background:#edeff1">1º Vogal</td>                
        <td><B>{{$notaInformativa->vogal_1}}</B></td>                
        <td style="background:#edeff1">2º Vogal</td>                
        <td><B>{{$notaInformativa->vogal_2}}</B></td>                
    </tr>                   
<script>
